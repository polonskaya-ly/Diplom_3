...
class MainPageLocators:
    ...
    INGREDIENT_POP_UP = [By.XPATH, ".//section[1]/div[1]/div[contains(@class,'content')]"]
    ORDER_FEED = [By.XPATH, ".//p[contains(text(),'Лента Заказов')]"]
    BASKET = [By.XPATH, ".//ul[contains(@class,'BurgerConstructor_basket')]"]
    INGREDIENT_COUNTER = [By.XPATH, ".//h2[contains(text(),'Булки')]/parent::div/ul/a//"
    ...
    ORDER_POP_UP_MESSAGE = [By.XPATH, ".//p[contains(text(),'Ваш заказ начали готовить')]"]
    ORDER_IDENTIFICATOR_MESSAGE = [By.XPATH, ".//p[contains(text(),'идентификатор заказа')]"]
    ORDER_NUMBER = [By.XPATH, ".//p[contains(text(),'идентификатор заказа')]/parent::div/h2"]
...
class FeedPageLocators:
    ...
    TOTAL_ORDERS = [By.XPATH, ".//p[contains(text(),'Выполнено за все время')]/parent::div/"
                              "p[contains(@class, 'OrderFeed_number')]"]
    TOTAL_ORDERS_CURRENT = [By.XPATH, ".//p[contains(text(),'Выполнено за все время')]/parent::div/"
                              "p[contains(@class, 'OrderFeed_number')]"]
    TODAY_ORDERS = [By.XPATH, ".//p[contains(text(),'Выполнено за сегодня')]/parent::div/"
                              "p[contains(@class, 'OrderFeed_number')]"]

    READY_MESSAGE_FOR_ORDERS = [By.XPATH, "//li[contains(text(),'Все текущие заказы готовы!')]"]


class OrderHistoryPageLocators:
    ORDER_NUMBER = [By.XPATH, ".//li[contains(@class, 'OrderHistory_listItem')]//p[contains(@class, 'text_type_digits')]"]
